	...
	Meteor.methods({
	  ...
	  attendeeList: function(event_id) {
	  	...
	  	for (var i = 0; i < attending.length; i++){
	  		...
	  		var add_attendee = {'fullname': attendee.profile.fullname,'organization':attendee.profile.organization,'email': attendee.emails[0].address}
	  		attendeeList.push(add_attendee)
	  	}
	  	return attendeeList;
	  },
	  attendee: function(email) {
	  	var attendee = Meteor.users.findOne({"emails.address": email})
	  	var profile = attendee.profile;
	  	var attendee_visible = {'fullname': profile.fullname, 'gender': profile.gender, 'organization': profile.organization, 'email': attendee.emails[0].address, 'phone': profile.phone}
	  	return attendee_visible;
	  },
	  eventHost : function(event_id){
	  	...
	  	var hostInfo = {'name': host.profile.fullname, 'organization': host.profile.organization, '_id':hostId}
	  	return hostInfo
	  }
